...
import com.dooques.fightingflow.ui.userScreen.abstractedFunctions.createAccountByEmail
...
fun EmailCreateUserDialog(
    ...
    val dobPicker = rememberDatePickerState()
    val selectedDob = dobPicker.selectedDateMillis?.let { convertMillisToDate(it) } ?: ""
        ...
        Card(
            ...
            Column(
                ...
                OutlinedTextField(
                    ...
                    isError = showDobError || showUserTooYoung,
                ...
                Row(horizontalArrangement = Arrangement.SpaceEvenly, modifier = modifier.fillMaxWidth().padding(top = 8.dp)) {
                    OutlinedButton(
                        onClick = {
                            Timber.d(" Launching User Creation Process\n User Details: \n Username: %s" +
                                    "\n Name: %s\n Email: %s\n Password: %s\n Confirm Password: %s\n DOB: %s",
                                username, name, email, password, confirmPassword, selectedDob)
                            scope.launch {
                                try {
                                    val accountCreationResult = createAccountByEmail(
                                        authViewModel = authViewModel,
                                        userViewModel = userViewModel,
                                        snackbarHostState = snackbarHostState,
                                        username = username,
                                        ...
                                        showDobError = { showDobError = true },
                                    )
                                    Timber.d("Successfully created user account for $username, closing dialog window.")
                                    if (accountCreationResult.isSuccess) {
                                        onDismissRequest()
                                    } else {
                                        throw accountCreationResult.exceptionOrNull() ?: Exception("Unknown error during account creation")
                                    }
                                } catch (e: Exception) {
